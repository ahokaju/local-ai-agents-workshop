"""
Kata 04: Local RAG - Bedrock Solution

This script mirrors solution.py but uses AWS Bedrock as the inference provider
instead of the Anthropic API directly.

Prerequisites:
    pip install llama-index llama-index-llms-bedrock llama-index-embeddings-huggingface chromadb boto3 python-dotenv
...
def create_query_engine(index, llm=None):
    """
    Create a query engine with a Bedrock-hosted Claude model as the LLM.
...
def main():
    """Run the RAG demo."""
    print("=" * 70)
    print(" Kata 04: Local RAG with LlamaIndex - Bedrock Solution")
